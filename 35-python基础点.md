    ...
    **模块和包
        ...
        2：包
            ...
            2-1:制作包
                ...
                注意：新建包后，包内部自动创建__init__.py文件，这个文件控制着包的导入行为

            2-2：导入包
                方法一：
                    导入包方式：import 包名.模块名
                    调用方式：包名.模块名.目标函数功能方法

                方法二：必须在__init__.py(控制着包的导入行为)文件中添加__all__ = [],控制允许导入的模块列表
                    导入包方式： from 包名 import * (通过这种方式导入包，必须要在__init__.py文件中添加模块列表，否则会报错)
                    调用方式：模块名.目标函数功能方法
