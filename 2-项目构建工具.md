...
**webpack
    ...
    2-作用：
        2-1：语法转换：
            ...
            es6转成es5
        2-2：html/css/js代码压缩合并打包
        2-3：webpack可以在开发期间提供一个开发环境
            自动打开浏览器
            保存时自动刷新
        2-4：项目一般先打包再上线

    3-用法：
        3-1:新建目录dist src/main.js(dist目录是我们打包后的代码，src是我们开发时的目录)

        3-2:初始化：npm init -y (生成的package.json可以记录我们下载的包)

        3-3:安装依赖包：npm i webpack webpack-cli -D

        3-4:到package.json文件中，配置scripts
            ```js
                scripts:{
                    'build':"webpack --config webpack.config.js"
                }
            ```
        3-5:在根目录新建一个webpack.config.js文件
            ...
            ```js
                ...
                module.exports = {
                    ...
                    mode:'development' //模式mode 为development是打包时未压缩，production打包时压缩
                }
            ```

        3-6：执行配置的scripts脚本：npm run build

        3-7：自动生成html的插件：html-webpack-plugin插件
            下载：npm i html-webpack-plugin -D
            在webpack.config.js文件中引入这个模块
                ```js
        ...
        3-7：配置css-loader(原因：webpack只认识json,javascript，不认识其他文件，如果希望打包处理其他文件，需要配置对应的loader)
        ...
        3-8：分离css文件：插件mini-css-extract-plugin插件。(将css从js文件中分离出来)
            作用：配置完运行之后，自动生成css文件，自动添加到index.html里
