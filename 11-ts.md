...
**模块
    1-导出
        1-1：导出声明
                1-1-1：定义：任何声明(比如变量，函数，类，类型别名或接口)都能通过添加export关键字来导出
                1-1-2：用法：
                        1-1-2-1：一个模块可以包裹多个模块，并把他们导出的内容联合在一起通过语法
                                事例：
                                ```ts
                                    export * from "module"
                                ```

        1-2:export=和import=require()
            export=：
                定义：export=语法定义一个模块的导出对象。它可以是类，接口，命名空间，函数或枚举
            若要导入一个使用了export=的模块时，必须使用typescript提供的特定的语法：import module = require('module')
            事例：
            ```ts
                // 在zip模块里
                ...
                // 在test.ts里，我们需要导入zip模块里的ZipCodeValidator
            ...
            ```
                导出导入事例执行逻辑：导出文件用export=形式导出，导入就必须要用import = require()导入

    2-生成模块代码
        概念：根据编译时指定的模块目标参数，编译器会生成相应的供Node.js,Require.js(AMD),isomorphic(UMD),SystemJS或ES6模块加载系统使用的代码
