    ...
    **Promise的基础知识
        ...
        3-Promise的生命周期
            ...
            3-3：Promise内部属性[[PromiseState]]
                3-3-1:概念：
                    内部属性[[PromiseState]]被用来表示promise3种状态：'pending','fulfilled','rejected'。这个属性不暴露在Promise对象上，所以不能以编程的方式检测Promise的状态，只有当Promise的状态改变时，通过then()方法来采取特定的行动
...
###代理(Proxy)和反射(Reflection)API
    ...
    **代理和反射
        1-定义：
            调用new Proxy()可创建代替其他目标对象的代理，它虚拟化了目标，所以二者看起来功能一致
        
        2-代理工作原理：
            代理可以连接js引擎内部目标的底层对象操作，这些底层操作被拦截后会触发响应特定操作的陷阱函数
        
        3-反射API概念：
            反射API以Reflect对象的形式出现，对象中默认的方法特性与相同的底层操作一致，一个陷阱函数对应一个反射API
        
    **创建一个简单的代理
        1-代理用法
            用Proxy构造函数创建代理需要传入两个参数：目标(target)和处理程序(handler)。处理程序是定义一个或多个陷阱对象，在代理中，除了专门为操作定义的陷阱外，其与操作均使用默认特性。不使用任何陷阱的处理程序等价于简单的转发代理
            事例：
            ...
            ```js
                ...
                proxy.name = 'proxy'
                console.log(proxy.name)  // 返回 'proxy'
                target.name = 'target'
                onsole.log(proxy.name) //返回 'target'
            ```
                事例执行逻辑：
                    这个示例中的代理将所有操作直接转发到目标，将'proxy'赋值给proxy.name属性时会在目标上创建name，代理只是简单地将操作转发给目标，它不会储存这个属性。由于proxy.name和target.name引用的都是target.name因此二者的值相同，从而为target.name设置新值后，proxy.name也一同变化
    
    **使用set陷阱验证属性
        概念：set代理陷阱可以拦截写入的属性的操作
        1-用法：
            1-1：set陷阱接受4个参数
                    参数一：trapTarget用于接收属性(代理的目标)的对象
                    参数二：key要写入的属性键(字符串或Symbol类型)
                    参数三：value被写入属性的值
                    参数四：receiver操作发生的对象(通常是代理)
            
            1-2：Reflect.set()
                概念：set陷阱对应的反射方法和默认特性，它和set代理陷阱一样也接受相同的4个参数。Reflect.set()方法基于操作是否成功来返回恰当的值
            ...
            1-3事例：
                ```js
                    ...
                    let proxy = new Proxy(target,{
                        set(trapTarget,key,value,receiver){
                            // 忽略不希望受到影响的已有属性
                            if(!trapTarget.hasOwnProperty(key)){   //hasOwnProperty()检测目标是否有键名 
                                if(isNaN(value)){  //isNaN()检测参数是否是非数值，是非数值返回true，是数值返回false
                                    throw new TypeError('属性必须是数字')
                                }
                            }

                            return Reflect.set(trapTarget,key,value,receiver) //反射API返回恰当的值
                        }
                    })
                    //添加一个新的属性
                    proxy.count = 1
                ...
                ```
                事例解释：这段代码定义了一个代理来验证添加到target的新属性，当执行proxy.count = 1时，set陷阱被调用，此时trapTarget的值等于target,key等于‘count’,value等于1，receiver(本例中未使用)等于proxy.由于target上没有count属性，因此代理继续将value值传入isNaN()，最后代理调用Reflect.set()方法并传入陷阱接受的4个参数来添加新属性

    **用get陷阱验证对象结果
        概念：get代理陷阱可以拦截读取属性的操作，只有当读取属性时才能检验属性
        1-用法：
            get陷阱接受3个参数
                参数一：trapTarget 被读取属性的源对象 (代理的目标)
                参数二：key 要读取的属性键 （字符串或Symbol）
                参数三：receiver 操作发生的对象(通常是指代理)
            
        2-反射API用法：Reflect.get()也接受3个参数并返回属性的默认值，如果属性在目标上不存在，则使用get陷阱和Reflect.get()时会抛出错误
            事例：
            ```js
                let proxy = new Proxy({},{
    ...
    **使用has陷阱隐藏已有属性
        概念：在代理中使用has陷阱可以拦截这些in操作并返回一个不同的值
        1-用法：
            参数一：trapTarget 读取属性的对象 (代理目标)
            参数二：key 要检查的属性键 (字符串或Symbol)
        
        2-对应的反射API Reflect.has()方法也接受这些参数并返回in操作符默认响应
        事例：检查对象上是否有value属性键
        ```js
            let target = {
                name:'target',
        ...
        ```
            事例解释：代理中的has陷阱会检查key是否为'value'，如果是的话返回false，若不是则调用Reflect.has()方法返回默认行为。结果是，即使target上实际存在value属性，但用in操作符检查还是会返回false，而对于name和toString则返回true

    **用deleteProperty陷阱防止删除属性
        1-在严格模式下删除不可配置属性会导致程序报错，在非严格模式下只会返回false
            事例：
            ```js
                let target = {
                    name:'target',
                ...
                Object.defineProperty(target,'name',{configurable:false}) //设置name为不可配置属性
                let result1 = delete target.value
                console.log(result1)  //返回true
                //注意在严格模式下，下面这行代码会报错，因为已经设置了name属性为不可配置属性
                let result2 = delete target.name   
                console.log(result2) //返回false
            ```
        
        2-deleteProperty陷阱的使用
            用法：
                参数一：trapTarget 要删除属性的对象(代理的目标)
                参数二：key要删除的属性键 (字符串或Symbol)
        
        3-对应的反射API Reflect.deleteProperty()方法为deleteProperty陷阱提供默认实现，并同样是接受两个参数

        事例：保证对象中value属性不会被删除
            ```js
                let target = {
                    name:'target',
                    value:42
                ...
                let proxy = new Proxy(target,{
                    deleteProperty(tarpTarget,key){
                        if(key === 'value'){
                            ...
                            return Reflect.deleteProperty(tarpTarget,key)
                        }
                    }
                })
                //尝试删除proxy.value
                let result1 = delete proxy.value
                console.log(result1) //返回false
                // 尝试删除proxy.name
                let result2 = delete proxy.name
                console.log(result2) //返回true
            ```

    **原型代理陷阱
        1-setPropertyOf
            用法：
                参数一：trapTarget 接受原型设置的对象 (代理的目标)
                参数二：proto 作为原型使用的对象
            1-1：Object.getPrototypeOf()方法
                    用法：Object.getPrototypeOf(obj,prototype)
                        参数obj：将被设置原型的对象
                        参数prototype:该对象新的原型 (是一个对象或null)
            1-2：Reflect.setPrototypeOf()方法和Object.setPrototypeOf()方法均是以上两个参数

        2-getPropertyOf
            用法：只接受一个参数 trapTarget (代理对象)
            Reflect.getPrototypeOf()方法和Object.getPrototypeOf()方法只接受trapTarget参数
        
        3-原型代理陷阱的运行机制
            3-1：getPrototypeOf陷阱运行机制：
                    getPrototypeOf陷阱必须返回对象或null，只要是返回值必将导致运行错误，返回值检查可以确保Object.getPrototypeOf()返回的总是预期的值

            3-2：setPrototypeOf陷阱
                    如果操作失败则返回的一定是false，此时Object.setPropertyOf()会抛出错误，如果setPrototypeOf返回了任何不是false的值，那么Object.setPrototypeOf()便假设操作成功
                    事例：
                    ```js
                        let target = {}
                        let proxy = new Proxy(target,{
                            getPrototypeOf(tarpTarget){
                                return false
                            },
                            setPrototypePf(trapTarget,proto){
                                return false
                            }
                        })
                        ...
                        Object.setPrototypeOf(proxy,{})  //会抛出错误
                    ```

            3-3：注意点：setPrototypeOf陷阱对应Object.setPrototypeOf操作
                       getPrototypeOf陷阱对应Object.getPrototypeOf操作
                ...
                ```js
                    ...
                    let proxy = new Proxy(target,{
                        getPrototypeOf(tarpTarget){
                            return Reflect.getPrototypeOf(tarpTarget)
                        },
                        setPrototypePf(trapTarget,proto){
                            return Reflect.setPrototypePf(trapTarget,proto)
                        }
                    })
                    let targetproto = Object.getPrototypeOf(target)
                    let proxyproto = Object.getPrototypeOf(proxy)
        ...
        4-为什么有两组方法
            原因：Reflect.getPrototypeOf和Object.getPrototypeOf存在着重要差异

            4-1：Reflect.getPrototypeOf和Object.getPrototypeOf的区别
                如果传入的参数不是对象，则Reflect.getPrototypeOf()方法会抛出错误，而Object.getPrototypeOf()方法则会在操作执行前先将参数强制装换为一个对象
                事例：
                ```js
                    let result1 = Object.getPrototypeOf(1)
                    console.log(result1 === Number.prototype)  //返回true 说明在操作之前先将参数强制转换为一个对象
                    Reflect.getPrototypeOf(1) //会抛出错误
                ```
                事例解释：Object.getPrototypeOf()方法会强制让数字1变为Number对象，所以你可以检索它的原型并得到返回值Number.prototype;而由于Reflect.getPrototypeOf()方法不会强制转化值的类型，而且1又不是一个对象，故会抛出错误

            4-2：Reflect.setPrototypeOf()和Object.setPrototypeOf()的区别
                Reflect.setPrototypeOf()方法返回一个布尔值来表示操作是否成功，成功时返回true，失败时返回false；而Object.setPrototypeOf()方法一旦失败则会抛出错误  查看上面3-2的例子

            4-3：代理陷阱中要使用Reflect上的方法，不要使用Object上的方法，虽然这两种方法的差别很小
                原因：看事例，然后看事例说明
                ```js
                    let target1 = {}
                    let result1 = Object.setPrototypeOf(target1,{})
                    cosole.log(result1 === target1) //Object.setPrototypeOf返回值是target1
                    let target2 = {}
                    let result2 = Reflect.setProrotypeOf(target2,{})
                    console.log(result2) // 返回true 
                ```
                事例解释：Object.setPrototypeOf()返回target1,但Reflect.setProrotypeOf()返回的是true。这种微妙的差异非常重要。在Object和Reflect上还有更多看似重复的方法，但是在所有代理陷阱中一定要使用Reflect上的方法


    **对象可扩展性陷阱
        1-概念：    
            es6中通过代理中的preventExtensions和isExtensible陷阱拦截Object.preventExtensions()方法和Object.isExtensible()方法并调用底层对象。
            1-1：用法：
                两个陷阱都接受唯一参数trapTarget对象，并调用它上面的方法。isExtensible陷阱返回的一定是一个布尔值，表示对象是否可扩展；preventExtensions陷阱返回的也一定是布尔值，表示操作是否成功。
            1-2：反射用法：
                Reflect.preventExtensions()方法和Reflect.isExtensible()方法实现了相应陷阱中的默认行为，二者都返回布尔值

            1-3：Object.preventExtensions()方法
                定义：让一个对象变的不可扩展，也就是永远不能添加新属性，并且返回原对象
                用法：Object.preventExtensions(obj)
                    参数obj：将要变得不可扩展的对象。
                    返回值：返回已经不可扩展的对象
            
            1-4：Object.isExtensible()
                定义：判断一个对象是否可扩展
                用法：Object.isExtensible(obj)
                    参数obj：需要检测的对象
                    返回值：表示给定对象是否可扩展的一个Boolean
        
        2-基础案例：让Object.preventExtensions()对于proxy失效
            ```js
                let target = {}
                let proxy = new Proxy(target,{
            ...
            ```
                事例解释：这里的Object.preventExtensions(proxy)调用实际上被忽略了，这是因为preventExtensions陷阱返回false，所以操作不会转发到底层目标，Object.isExtensible()最终返回true

        3-重复的可扩展性方法
            3-1：高级方法Object.isExtensible()和底层方法Reflect.isExtensible()区别
                3-1-1：只有当传入非对象时，Object.isExtensible()返回false，而Reflect.isExtensible()则抛出一个错误
                    事例：
                    ```js
                        let result1 = Object.isExtensible(2)
                        console.log(result1)  //false

                        let result2 = Reflect.isExtensible(2) //会抛出错误
                    ```
                
                3-1-2：相比较高级功能方法而言，底层的具有更严格的错误检查。无论传入Object.preventExtensions()方法的参数是否为一个对象，它总是返回该参数；而如果Reflect.preventExtensions()方法参数不是对象就会抛出错误；如果参数是一个对象，操作成功时Reflect.preventExtensions()会返回true，否则返回false
                    事例：
                    ```js
                        let result1 = Object.preventExtensions(2)
                        console.log(result1)  //返回2

                        let target = {}
                        let result2 = Reflect.preventExtensions(target)
                        console.log(result2)  //返回true
                        let result3 = Reflect.preventExtensions(2) //抛出错误
                    ```
                
    
    **属性描述符陷阱
        1-defineProperty陷阱
            1-1：概念：defineProperty陷阱拦截的是Object.defineProperty()方法调用

            1-2：用法：
                参数一：trapTarget 要定义属性的对象 (代理的目标)
                参数二：key 属性的键 (字符串或Symbol)
                参数三：descriptor 属性的描述符对象
                返回值：defineProperty陷阱需要在操作成功后返回true，否则返回false

            1-3：反射API：Reflect.defineProperty()方法接受的参数跟对应的陷阱函数接收的参数相同

            事例：如果希望阻止Symbol类型的属性，则可以当属性键为Symbol时返回false
            ```js
                let proxy = new Proxy({},{
                ...
                Object.defineProperty(proxy,nameSymbol,{  //会报错
                    value:'proxy'
                })
            ```

        2-getOwnPropertyDescriptor陷阱 (注意返回值)
            2-1用法：
                参数一：trpTarget 要定义属性的对象 (代理的目标)
                参数二：key 属性的键 (字符串或Symbol)

                返回值：返回值必须是null，undefined或一个对象；如果返回对象，则对象自己的属性只能是enumerable,configurable,value,writable,get,set,在返回的对象中使用不被允许的属性会抛出错误
                事例：
                ```js
                    let proxy = new Proxy({},{
                ...
                ```
                    事例解释：属性描述符中不允许有name属性，当调用Object.getOwnPropertyDescriptor()时。getOwnPropertyDescriptor的返回值触发一个错误。
            
            2-2反射API:Reflect.getOwnPropertyDescriptor接收的参数跟对应的陷阱函数参数一样
            事例：
            ```js
                let proxy = new Proxy({},{
                    defineProperty(trapTarget,key,descriptor){
                ...
                // obj参数:要在其中查找属性的对象。
                // prop参数：检索其描述的名称或属性
                // 返回给定对象的自身属性（即，直接存在于对象上而不是对象原型链中的一个属性描述符）
                let descriptor = Object.getOwnPropertyDescriptor(proxy,'name')
                console.log(descriptor.value) //返回'proxy'
                ```

        3-描述符对象限制
            概念：无论将什么对象作为第三个参数传递给Object.defineProperty()方法，都只有属性enumerable,configurable,value,writable,get和set将出现在传递给defineProperty陷阱的描述符对象中
                概念解释：这些属性在陷阱中是允许引用的标准属性，如果是非标准属性会被反射API和descriptor忽略掉
                事例：
                ```js
                    let proxy = new Proxy({},{
                        defineProperty(trapTarget,key,descriptor){
                ...
                ```
                    事例解释：在这段代码中，调用Object.defineProperty()时传入包含非标准name属性的对象，作为第三个参数。当defineProperty陷阱被调用时，descriptor对象有value属性却没有name属性，这是因为descriptor不是实际传入Object.defineProperty()方法的第三个参数的引用，而是一个只包含那些被允许使用的属性的新对象。Reflect.defineProperty()方法同样也忽略了描述符上的所有的非标准属性
            
        4-重复的描述符方法
            4-1：defineProperty()方法
                概念：Object.defineProperty()方法Reflect.defineProperty()方法只有返回值不同：
                Object.defineProperty()方法返回第一个参数，而Reflect.defineProperty()的返回值与操作有关，成功则返回true，失败则返回false
                事例：
                ```js
                    let target = {}
                    ...
                    console.log(result2) // true 则表示操作成功
                ```

            4-2：getOwnPropertyDescriptor()方法
                概念：调动Object.getOwnPropertyDescriptor()方法时传入原始值(这里指数字)作为第一个参数，内部会将这个值强制转换为一个对象；另一方面，若调用Reflect.getOwnPropertyDescriptor()方法时传入原始值作为第一个参数，则会抛出一个错误
                事例：
                ```js
                    let descriptor1 = Object.getOwnPropertyDescriptor(2,'name')
                    console.log(descriptor1)  //undefined

                    let descriptor1 = Reflect.getOwnPropertyDescriptor(2,'name') //抛出错误
                ```
                    事例解释：由于Object.getOwnPropertyDescriptor()方法将数值2强制转换为一个不含name属性的对象，因此它返回undefined；然而点调用Reflect.getOwnPropertyDescriptor()时立即抛出一个错误，因为该方法不接受原始值作为第一个参数

    
    **ownKeys陷阱
        1-概念：
            ownKeys代理陷阱可以拦截内部方法[[OwnPropertyKeys]],我们通过返回一个数组的值可以覆写其行为。这个数组被用于Object.keys(),Object.getOwnPropertyNames(),Object.getOwnPropertySymbols(),Object.assign()4个方法，Object.assign()方法用数组来确定需要赋值的属性
            1-1：概念说明：这4种方法，用ownKeys陷阱拦截内部方法[[ownPropertykeys]]覆盖着4种方法的行为
            1-2：用法：
                ownKeys陷阱唯一接受的参数是操作的目标，返回值必须是一个数组或类数组对象，否则就抛出错误。当调用Object.keys(),Object.getOwnPropertyNames(),Object.getOwnPropertySymbols(),Object.assign()方法时，可以用ownKeys陷阱来过滤掉不想使用的属性键。
                事例：
                ```js
                let proxy = new Proxy({},{
                    ownKeys(trapTarget){
            ...
            ```
                事例：这个事例使用了一个ownKeys陷阱，它首先调用Reflect.ownKeys()获取目标的默认键列表；接下来，用filter()过滤掉以下下划线字符开始的字符串；然后，将3个属性添加到proxy对象：name,_name和nameSymbol。调用Object.getOwnPropertyNames()和Object.keys()时传入proxy，只返回name属性；同样，调用Object.getOwnPropertySymbols()时传入proxy，只返回nameSymbol

        2-反射API：ownKeys陷阱通过Reflect.ownKeys()方法实现默认的行为

        3-注意点：ownkeys陷阱也会影响for-in循环，当确定循环内部使用的键时会调用陷阱

    **函数代理中的apply和construct陷阱
        1-概念：函数有两个内部方法[[Call]]和[[Construct]]，apply陷阱和construct陷阱可以覆写这些内部方法。
        2-执行机制：若使用new操作符调用函数，则执行[[Construct]]方法；若不用，则执行[[Call]]方法
        3-apply陷阱用法：
            参数一：trapTarget 被执行的函数 (代理的目标)
            参数二：thisArg 函数被调用时内部this的值
            参数三：arguments 传递给函数的参数数组

            3-1：Reflect.apply()和apply()同样接受以上三个参数
        
        4-使用new调用函数时调用的construct陷阱用法
            参数一：trapTarget 被执行的函数 (代理的目标)
            参数二：arguments 传递给函数的参数数组

            4-1：Reflect.construct()方法同样接受这俩个参数，其还有一个可选的第三个参数newTarget。若给定这个参数，则该参数用于指定函数内部new.target的值
        
        5-验证函数参数
            事例：假设你想验证所有参数都属于特定类型，则可以在apply陷阱中检查参数
            ```js
                function sum(...values){
                ...
                let sumProxy = new Proxy(sum,{
                    ...
                    construct:function(trapTarget,argumentList){
                        throw new TypeError('该函数不可以通过new来刁红')
                    }
                })
            ```

            事例：确保必须用new来调用函数并验证其参数必须数字
            ```js
                ...
                let instance = new numbersProxy(1,2,3,4,5)
                console.log(instance.value) //返回 [1,2,3,4,5]
            ```
        ...
        6-不用new调用构造函数
            ...
            ```
                事例解释：apply陷阱用传入参数调用Reflect.construct,就可以让numberproxy()函数无需调用new就能实现用new调用numbers的行为
                    事例的进一步解释：当调用new时触发construct()陷阱，Reflect.constructor()是事件new的默认行为并返回值
                        
        7-覆写抽象基类构造函数
            事例：用代理给new.target赋值来绕过构造函数限制
            ```js
                ...
                let AbstrackNumbersProxy = new Proxy(AbstractNumbers,{
                    construct:function(trapTarget,argumentList){
                        return Reflect.construct(trapTarget,argumentList,function(){})
                    }
                })
                let instance = new AbstrackNumbersProxy(1,2,3,4)
                console.log(instance.value) //返回[1,2,3,4]
            ```
            事例解释：AbstrackNumbersProxy使用construct陷阱来拦截对new AbstrackNumbersProxy()方法的调用。然后传入陷阱的参数来调用Reflect.construct()方法，并添加一个空函数作为第三个参数。这个空函数被用作构造函数内部new.target的值，由于new.tareget不等于AbstractNumbers，因此不会抛出错误

            事例执行机制：Reflect.construct()第三个参数为可选参数用于指定函数内部new.target的值。这里将AbstractNumbers类的new.target指向function(){}

        8-可调用的类构造函数
            事例：如果希望类构造函数不用new就可以运行，可以使用apply陷阱来创建一个新实例(不用new调用构造函数实例)
            ```js
                class person {
                    constructor(name){
                        this.name = name
            ...
            ```
                事例解释：apply陷阱覆写默认行为并返回trapTarget的新实例，该实例与person相等


    **可撤销代理
        1-概念：通常情况下，在创建代理后，代理不能脱离其目标(也就是不可撤销代理)。但是可能存在你想撤销代理的情况，无论是出于安全目的通过API提供对象，还是在任意时间点切断访问，你将发现撤销代理非常有用

        2-用法：使用Proxy.revocable()方法创建可撤销的代理，该方法采用与Proxy构造函数相同的参数：
                参数一：proxy 可被撤销的代理对象
                参数二：撤销代理要调用的函数

            2-1：当调用revoke()函数时，不能通过proxy执行进一步的操作。任何与代理对象交互的尝试都会触发代理陷阱抛出错误
                事例：
                ```js
                    let target = {
                        name:'target'
                    }
                    // 不可撤销代理new Proxy(target,{})和可撤销代理Proxy.revocable(target,{})用法是一样的
                    let {proxy,revoke} = Proxy.revocable(target,{})
                    console.log(proxy.name) //'target'
                    revoke()
                    console.log(proxy.name) 
                    //在调用revoke()方法之后，proxy对象不再是可用的代理对象，尝试访问proxy.name将会抛出错误
                ```
    ...
    **解决数组问题
        ...
        4-实现MyArray类
            ...
            4-1：概念：想要创建代理的类，最简单的方法是像往常一样定义类，然后在构造函数中返回一个代理，那样的话，当类实例化时返回的对象是代理而不是实例

            4-2：代理类的目的：事例完全私有化，除了通过代理间接访问外，无法直接访问它
                事例：
                ```js
                    class Thing {
                        constructor(){
                    ...
                    console.log(myThing instanceof Thing) //返回true
                ```
                    事例解释：Thing实例对象只有通过代理访问，因为new调用Thing时返回的是一个代理

    **将代理用作原型
        注意点：必须要让代理作为原型，才会触发陷阱
        事例：
        ```js
            function my(){}
            my.prototype.name = new Proxy({},{
                getprototype(trapTarget,key){}
            }) 
            //给my函数方法上添加name原型，name属性对应的值是代理，比如用get的查询机制，是查询不到这个代理的,因为这里没有让代理作为原型
        ```
        1-概念：能让所有实例共享一个代理

        2-代理是原型的缺点：代理是原型，仅当默认操作继续执行到原型上时才会调用代理陷阱，这会限制代理作为原型的能力
            事例：
            ```js
                let target = {}
                // Object.create(Obj) 参数obj：新创建对象的原型对象。
                let newTarget = Object.create(new Proxy(target,{
                    defineProperty(trapTarget,name,descriptro){
                        return false
                    }
                }))

                <!-- 调用Object.defineProperty()方法并传入newTarget来创建一个名为name的自有属性。在对象上定义属性的操作不需要操作对象原型，所以代理中的defineProperty陷阱永远不会被调用 -->
        ...
        3-在原型上使用get陷阱
            3-1：get执行机制：调用内部方法[[Get]]读取属性的操作先查找自有属性；如果未找到指定名称的自有属性，则继续到原型查找，直到没有更多可以查找的原型过程结束
            ...
            ```js
                ...
                thing.name = "thing"
            ```
            3-1-1：事例中的trapTarget和receiver是不同的对象解释：当代理被用作原型时，trapTarget是原型对象，receiver是实例对象。在这种情况下，trapTarget和target相等，receiver与thing相等，所以可以访问代理的原始目标和要操作的目标

        4-在原型上使用set陷阱
            4-1：set执行机制：内部方法[[Set]]同样会检查目标对象中是否有某个自有属性，如果不存在则继续查找原型。当给对象属性赋值时，如果存在同名自有属性则赋值给它；如果不存在给定名称，则继续在原型上查找。最棘手的是，无论原型上是否存在同名属性，给该属性赋值时都默认在实例中创建该属性
            事例：
            ```js
                let target = {}
                let thing = Object.create(new Proxy(target,{
                ...
                console.log(thing.hasOwnProperty('name')) //返回false表示thing对象上没有name属性
                thing.name = 'thing' //触发set陷阱
                console.log(thing.hasOwnProperty('name'))//返回true
                thing.name = 'boo' //因为thing自有属性上已经有了name属性，所以不会再触发set陷阱
            ```
                事例解释：在陷阱中trapTarget和target相等，receiver与thing相等.最终该操作会在thing上创建一个新的属性，很幸运，如果传入receiver作为第四个参数，Reflect.set()就可以实现这个默认行为(默认行为是：给该属性赋值时都默认在实例中创建该属性)(原理：receiver能够在thing对象上创建属性)

        5-在原型上使用has陷阱
            5-1：has执行机制：它可以拦截对象中的in操作符。in操作符先根据给定名称搜索对象的自有属性，如果不存在，则沿着原型链一次搜索后续对象的自有属性，直到找到给定的名称或无更多的原型为止
            事例：
            ```js
                let target = {}
                let thing = Object.create(new Proxy(target,{
                ...
                console.log('name' in thing) //触发has代理陷阱
                thing.name = 'thing'
                console.log('name' in thing)) //不会触发代理陷阱
            ```
        
        6-将代理用作类的原型

            6-1：原因：由于类的prototype属性是不可写的，因此不能直接修改类来使用代理作为类的原型。然而可以通过继承的方法来让类误以为自己可以将代理用作自己的原型

            6-2：用法：需要用构造函数创建一个es5风格的类型定义
            事例：
            ```js
                ...
                let shap = new Square(2,6)
                let are1 = shap.length*shap.width
                let are2 = shap.length*shap.wod //由于wod属性不存在所以，代理会报错
            ```
                6-2-1：注意点：事例对象shap的原型的原型时proxy代理
        

##用模块封装代码
    1-概念：es6为了解决js代码的使用量增长，导致的命名冲突和安全问题，所以出现了模块化封装代码

    **什么是模块
        1：概念：模块是自动运行在严格模式下的并且没有办法退出运行的js代码
        2：模块的作用：模块的真正魔力所在是仅导出和导入你需要的绑定，而不是将所有东西都放到一个文件。只有很好的理解了导出和导入才能理解模块与脚本的区别
    
    **导出的基本语法
        1：概念：exprot关键字将一部分已发布的代码暴露给其他模块
        2：用法：将export放在任何变量，函数或类声明的前面
        ```js
            // 导出数据
            export var color = 'red' 
            export let name = 'shihui'
            // 导出函数
    ...
    **导入的基本语法
        1：概念：从模块中导出的功能可以通过import关键字在另一个模块中访问
        2：语法：import语句的两个部分分别是：要导入的标识符和标识符应当从哪个模块导入
        事例：
        ```js
            import {identifier1,indetifier2} from './example.js'
        ```
            事例解释：import后面的大括号表示给定模块导入的绑定，关键字from表示从哪个模块导入给定的绑定，该模块由表示模块路径的字符串指定

        3：注意点：
            3-1：nodejs遵循基于文件系统前缀区分本地文件和包的惯例：例如，example是一个包，而./example.js是一个本地文件
            3-2：导入绑定的列表看起来与解构对象很相似，但它不是
        
        4：模块中导入绑定的特性：
            它就好像使用了const定义的一样。结果是你无法定义另外一个变量，也无法在import语句前使用标识符或改变绑定的值


        5-导入单个绑定
            事例：
            ```js
                // 在example.js文件中有sum函数方法
                import {sum} from './example.js'
                console.log(sum(1,2)) //返回3
                sum = 1 //抛出错误，因为不能给导入的绑定重新赋值
            ```
            5-1：注意点：为了最好的兼容多个浏览器和nodejs环境，一定要在字符串之前包含/，./或../来表示要导入的文件
        
        6-导入整个模块
            6-1：概念：特殊情况下，可以导入整个模块作为一个单一的对象。然后所有的导出都可以作为对象的属性使用
            事例：
            ```js
                import * as example from './example.js'
                console.log(example.sum(1,2)) //返回3
            ```
                6-1-1：这种导入被称为命名空间导入。因为example.js不存在example对象，故而它作为example.js中所有导出成员的命名空间对象而被创建
                6-1-2：导入模块的执行机制：不管在import语句中把一个模块写了多少次，该模块将只执行一次。导入模块的代码执行后，实例化过的模块被保存在内存中，只要另一个import语句引用它就可以重复使用它
                事例：
                ```js
                    import {sum} from './example.js'
                    import {muitiply} from './example.js'
                    import {magicnumber} from './example.js'
                ```
                    6-1-2-1：事例解释：尽管在这个模块中有3个import语句，但example.js将只执行一次。如果同一个应用程序中的其他模块也从example.js导入绑定，那么那些模块与代码将使用相同的模块实例
            
            6-2：模块语法的限制：
                6-2-1：export和import的一个重要的限制是，它们必须在其他语句和函数之外使用
                事例：这个事例会给出一个语法错误
                ```js
                    if(flag){
                        export flag//语法错误
                ...
                ```
                事例解释：模块语法存在一个原因是要让js引擎静态地确定哪些可以导出。因此模块在顶部使用

                6-2-2：不能在一条语句中使用import，只能在顶部使用它
                事例：
                ```js
                    function tryImport(){
        ...
        7-导入绑定的一个微妙怪异之处
            概念：标识符只有在被导出的模块中可以修改，导入绑定的模块也无法更改绑定
            事例：
            ```js
                // example.js文件中
                ...
                // 导入的模块中
                import {name,setName} from 'ç'
                setName('greg')
                console.log(name) //从nicholas编程 'greg'
                name = 'shihui'  //抛出错误，因为无法更改绑定的值
            ```
                事例解释：通过import导入绑定时，可以理解成用了const定义导入绑定的模块
            
    **导出和导入时重命名
        1：要使用不同的名称导入一个函数，则可以用as关键字来指定函数在模块外应该被称为什么名称
        事例：
        ```js
            ...
            // 在另一个模块中导入时必须要使用add这个名称
            import {add as sum} from './example.js'
            // 这段代码导入add函数时使用了导入重命名成sum函数。导入时改变函数的本地名称意味着即使模块导入了add()函数，在当前模块中也没有add()标识符
        ```

    **模块的默认值
        1：模块默认值语法：
            模块默认值指的是通过default关键字指定的单个变量，函数或类，只能为每个模块设置一个默认的导出值，导出时多次使用default关键字是一个语法错误
        ...
        2：导出默认值：
            2-1：不要名称
                ...
                ```
            2-2:在export default之后添加默认导出值的标识符
                ```js
                    function sum(num1,num2){
                        return num1 + num2
                    }
        ...
        3:导入默认值
            ...
            ```
            事例解释：这条import语句从模块example.js中导入了默认值，请注意，这里没有使用大括号，与你见过的非默认导入的情况不同。 本地名称sum用于表示模块导出的任何默认函数，这种语法是最纯净的
    ...
    **无绑定导入
        1-概念：内建对象(Array和Object)的共享定义可以在模块中访问，这些对象所做的更改将反映在其他模块中
        事例：
        ```js
            Array.prototype.pushAll = function(items){
                ...
                return this.push(...items) //使用内建的push()和展开运算符
            }
        ```
        由于事例代码不导出任何东西，因而你可以使用简化的导入操作来执行模块代码，且不导入任何绑定
         1-1：代码如下
            ```js
                import './example.js'
                let colors = ['red','green','blue']
                let items = []
                items.pushAll(colors)  //将colors数组添加到items数组中
            ```
                这段代码导入并执行了模块中包含的pushAll方法，所以pushAll()被添加到数组的原型中，也就是说现在模块中的所有数组都可以使用pushAll()方法了

    
    **加载模块
        1-在web浏览器中使用模块
            概念：es6之前，js脚本加载方法
                1-1：在<script>元素中通过src属性指定一个加载代码的地址来加载js代码文件
                1-2：将js代码内嵌到没有src属性的<script>元素中
                1-3：通过web Worker或Service Worker的方法加载并执行js代码文件
        
        2-在<script>中使用模块
            2-1概念：type属性值为'module'时支持加载模块。将type设置为module可以让浏览器将所有内联代码或包含src指定的文件中的代码按照模块而非脚本的方式加载
            ```js
                //加载一个js模块文件
                ...
                <script>
            ```
                2-1-1:脚本和模块文件的区别是：模块与脚本的唯一区别是type值为module。定义在模块文件中的变量不会添加到全局(不会添加到window中)
            
            2-2：注意点：当无法识别type的值时，浏览器会忽略<script>元素，因此不支持模块的浏览器将自动忽略<script type="module">来提供良好的向后兼容性


        3-web浏览器中的模块加载顺序
            3-1：当type='module'时的特性：
                3-1-1：加载脚本文件时，defer是可选属性；加载模块时，它就是必须属性
                模块按照它们出现在html文件中的顺序执行，也就是说，无论模块中包含的是内联代码还是指定src属性，第一个<script type="=module">总是在第二个之前执行
                3-1-2：所有导入资源都被加载和执行后才会执行当前模块
        ...
        4-浏览器模块说明符解析
            浏览器要求模块说明符具有以下几种格式之一
                ...
                事例：以URL格式为例
                ```js
                    import {fouth} from 'https://www2.example.com/example4.js'
                ```
...
###es6中的较小的改动
    **使用整数
        1-识别整数
            Number.isInterger()方法：
                执行机制：当调用该方法并传入一个值时，js引擎会查看该值的底层表示方式来确定该值是否为整数
                用法：是整数返回true，否则返回false
                事例：
                ```js
                    Number.isInterger(25) //true
                    Number.isInterger(25.0) //true 数字看起来像浮点数，其实存储的是整数则返回true
                    Number.isInterger(25.1) //false
                ```
        
        2-安全整数
            2-1：概念：安全数范围：-Math.pow(2,53)到Math.pow(2,53)  (-2的53次方到2的53次方)
            2-2：Math.pow()用法
            2-3:Math.isSafeInterger()方法：识别数字是否在安全数之内
            2-4:Math.MAX_SAFE_INTERGER：整数范围的上限
            2-5:Math.MIN_SAFE_INTERGER：整数范围的下限

    
    **正式化__proto__属性
        1-概念：在es6之前__proto__是Object.getPrototypeOf()方法和Object.setPrototypeOf()方法的早期实现

        2：__proto__具有以下特征：
            2-1：只能在对象字面量中指定一次__proto__，如果指定两个__proto__属性则会抛出错误
            2-2：可计算形式的['__proto__']的行为类似于普通属性，不会设置或返回当前对象的原型

        3：使用__proto__和使用Object.getPrototypeOf()方法或Object.setPrototypeOf()方法的区别：
            ...
            ```js
                ...
                let friend = {
                    __proto__:person  //__proto__属性可以直接将person对象设置成friend的原型
                }
            ```

###es7
    es7的仅添加了三种新特性：新的数学运算符，新的数组方法，新的语法错误
    **指数运算符
        1-Math.pow()可以进行求幂运算
        2-求幂运算符：
            概念：是两个符号** ：左操作数是基数，右操作数是指数
            事例：
            ```js
                let result = 5**2
        ...
        3-运算顺序
            概念：求幂运算在js所有二进制运算符中具有最高的优先级(一元运算符的优先级高级** 一元运算符如：++count,--count)

        4-运算限制
            4-1：禁用求幂运算符左侧的一元表达式可以消除歧义。要明确指出意图，需要用括号包裹
                事例：
                ```js
                    let result1 = -(5**2) //等于-25
                    let result2 = (-5)**2 //等于25
                ```
            4-2：求幂运算符左侧无须用括号就可以使用++和--
                运行机制：前缀++或--会在其他所有操作发生之前更换操作数，而后缀版本(count++)直到整个表达式被计算过后才会进行改变。这两个方法在运算符左侧都是安全的
                事例：
                ```js
                    let num1 = 2;
                    let num2 = 2;
                    console.log(++num1**2) //返回9
                    console.log(num1) //返回3
                    console.log(num2--**2) //返回4  num2取幂运算的值保持为2，之后再减1
                    console.log(num2) //返回1
                ```

    **Array.prototype.includes()方法
        概念：给数组添加了inclueds方法
        1-如何使用Array.prototype.inclueds()方法
            用法：Array.prototype.inclueds()方法接受两个参数：
                    参数一：要搜索的值
                    参数二：开始搜索的索引位置(可选)，提供第二个参数时，includes()将从该索引开始匹配
                    返回值：boolean

        2-值的比较
            2-1值为NaN时的indexOf和includes比较：
                事例：
                ```js
                    let values = [1,NaN,2]
                    console.log(values.indexOf(NaN))  //返回-1表示匹配不到
                    console.log(values.includes(NaN)) //返回true
                ```

            2-2:值为+0和-0时indexOf(),includes(),Object.is()的比较
                indexOf(),includes()值为-0时都会找到+0
                Object.is(-0,+0) 返回值不同

    
    **函数作用域严格模式的一处改动
        1-es5中规定在参数被解构或有默认参数的函数中禁止使用'use strict'指令
        事例：
        ```js
            function okey(first,second){   //不会报错
                'use strict'
                return first
            }
            function notOkey(first,second = first){  //会抛出错误
                'use strict'
                return first
            }
            function notOkey2({first,second}){  //会抛出错误
                'use strict'
                return first
            }
        ```
